...
class ParameterManager(Manager['Parameter']):
    def random(self, column_type: ColumnType | str) -> Tuple[str, Optional[str]]:
    ...
    def get_character_set(self, deep: int = 0) -> dict[str, str]:
        try: 
            character_set: dict[str, str] = {}
            for column_type, display_name in ColumnType.choices:
                ...
                character_set[display_name] = title.format(**format_dict)
        except Exception as e:
            logger.error(f"Error generating character set: {e}")
            return self.get_character_set(deep=deep + 1) if deep < 5 else {}
        return character_set


class Parameter(Model):
    objects: ParameterManager = ParameterManager()
    title = CharField(
        max_length=100,
        blank=False,
        null=False,
